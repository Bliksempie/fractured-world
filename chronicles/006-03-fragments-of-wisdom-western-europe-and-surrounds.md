<!-- fragment-meta
    affects-living-chronicles: true
    converts-myth-to-confirmed: false
    fragment-id: fragment-singing-bridge-hum
    fragment-timing: earlygame
    fragment-lore-type: location
    unlock-behavior: on-acquisition
-->
### The Singing Bridge Hum

**Earlygame**: Audio snippet transcribed from a handheld recorder found under a collapsed viaduct in France.

> "It wasn’t the wind. It was a warning... tuned for someone who never came."
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: false
    converts-myth-to-confirmed: false
    fragment-id: fragment-vatican-signal-decode
    fragment-timing: midgame
    fragment-lore-type: signal
    unlock-behavior: on-analysis
-->
### Vatican Signal Decode

**Midgame**: Snippets from the 9-minute broadcast recovered by an old signal-bunker team.

> "Not Latin. Not human. Just a voice that *knew* us. And it wasn’t praying."
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: true
    converts-myth-to-confirmed: false
    fragment-id: fragment-glass-cathedral-of-london
    fragment-timing: lategame
    fragment-lore-type: structure
    unlock-behavior: on-analysis
-->
### The Glass Cathedral

**Lategame**: Recovered drone footage from a half-collapsed tower in the Drowned Quarter.

> "Stained glass cracked by spores—but still glowing. Still casting shadows that don’t match what’s in the room."
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: false
    converts-myth-to-confirmed: false
    fragment-id: fragment-danube-drowning
    fragment-timing: earlygame
    fragment-lore-type: myth
    unlock-behavior: on-acquisition
-->
### Danube Drowning

**Earlygame**: An old river song scribbled in charcoal on the inside of a boat hull.

> "The water rose not with rain, but with memory. She sings to keep the drowned from climbing back."
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: true
    converts-myth-to-confirmed: true
    fragment-id: fragment-paris-stalemate
    fragment-timing: midgame
    fragment-lore-type: event
    unlock-behavior: on-analysis
-->
### The Paris Stalemate

**Midgame**: Recovered from a time-locked vault in a metro station beneath Montparnasse.

> "Three factions. One causeway. No shots. No words. Just... waiting. For what, no one remembers."

*If analyzed, the Living Chronicle records one of the last non-violent pre-Fall conflicts, influencing survivor dialogue around memory, truce, and intent.*
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: true
    converts-myth-to-confirmed: true
    fragment-id: fragment-echoes-of-the-louvre
    fragment-timing: lategame
    fragment-lore-type: cultural
    unlock-behavior: on-analysis
-->
### Echoes of the Louvre

**Lategame**: Audio loop extracted from a secure museum drone's final broadcast.

> "You hear brushstrokes. You hear footsteps. You hear the Mona Lisa exhale."

*If analyzed, this fragment adds “cultural resonance” entries to the Living Chronicle—shaping survivor behavior around high-art ruins and echo spaces.*
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: true
    converts-myth-to-confirmed: true
    fragment-id: fragment-rhine-code
    fragment-timing: lategame
    fragment-lore-type: signal
    unlock-behavior: on-analysis
-->
### The Rhine Code

**Lategame**: Compiled from flickers in dying power relays along the German border.

> "The bursts translate to a countdown. Or maybe a clock. But the dates... they’re still ahead of us."

*If analyzed, the Living Chronicle gains a speculative “pending event marker,” changing survivor perception of future collapse phases.*
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: true
    converts-myth-to-confirmed: true
    fragment-id: fragment-cathedral-that-watched
    fragment-timing: midgame
    fragment-lore-type: structure
    unlock-behavior: on-analysis
-->
### The Cathedral That Watched

**Midgame**: Drawn from 23 testimonies in different cities—each describing the same spire.

> "It had no doors. No glass. And no shadow. But everyone who looked at it said it felt familiar."

*If analyzed, establishes a Living Chronicle marker for “shared hallucination structures” possibly tied to gravity pressure.*
<!-- /fragment-meta -->
