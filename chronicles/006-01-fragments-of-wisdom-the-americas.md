<!-- fragment-meta
    affects-living-chronicles: true
    converts-myth-to-confirmed: false
    fragment-id: fragment-denver-chorus
    fragment-timing: earlygame
    fragment-lore-type: location
    unlock-behavior: on-acquisition
-->
### Denver Chorus

**Earlygame**: A field recording stored on a cracked voice module recovered near the Exclusion Zone.

> "They said the Hum never left Denver. I say it never started here. This isn't the Hum. It's a choir of the buried trying to harmonize through dust."
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: false
    converts-myth-to-confirmed: false
    fragment-id: fragment-compass-of-bone
    fragment-timing: midgame
    fragment-lore-type: item
    unlock-behavior: on-analysis
-->
### Compass of Bone

**Midgame**: A crude compass fashioned from copper wire and human finger bones.

> "They said it always points to safety. But in the Gulf Scar... it spun."
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: true
    converts-myth-to-confirmed: false
    fragment-id: fragment-alaskan-rumor-line
    fragment-timing: midgame
    fragment-lore-type: person
    unlock-behavior: on-acquisition
-->
### Alaskan Rumor Line

**Midgame**: A survivor's transcript from intercepted comms between stations near the Broken Line.

> "We haven't seen the train, but we *hear* it. No whistle. Just the hum. And the ice cracking like it remembers something."
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: false
    converts-myth-to-confirmed: false
    fragment-id: fragment-saints-of-signal
    fragment-timing: earlygame
    fragment-lore-type: group
    unlock-behavior: on-acquisition
-->
### Saints of Signal

**Earlygame**: A bloodstained flyer nailed to a tech-shack wall in former Detroit.

> "When the towers fell, they kept listening. Said the old world was still speaking. Just not in words anymore."
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: true
    converts-myth-to-confirmed: true
    fragment-id: fragment-new-york-twice
    fragment-timing: lategame
    fragment-lore-type: location
    unlock-behavior: on-analysis
-->
### New York, Twice

**Lategame**: Found encrypted in a fractured map shard, accessible only after scanning multiple landmark fragments.

> "There is a second Manhattan. It's buried in frost and twilight, mirrored below the East River. They call it the Quiet Borough. Some say it never fell. Some say no one inside knows the world ended."

*If analyzed, this fragment modifies the Living Chronicle to acknowledge the "mirrored zone" theory, unlocking potential encounter threads tied to dual locations.*
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: true
    converts-myth-to-confirmed: true
    fragment-id: fragment-last-congress-broadcast
    fragment-timing: midgame
    fragment-lore-type: signal
    unlock-behavior: on-analysis
-->
### The Last Congressional Broadcast

**Midgame**: A damaged audio reel unearthed from a sealed fallout shelter near D.C.

> "We are not infected. We are not resigned. We are not gone. If you can hear this, the Bloom is not natural. We did not lose. We were *surrendered.*"

*If analyzed, this fragment marks a Living Chronicle shift suggesting high-level human complicity in the collapse, unlocking suspicion-based survivor dialogue and vault entries.*
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: true
    converts-myth-to-confirmed: true
    fragment-id: fragment-treaty-of-dust
    fragment-timing: lategame
    fragment-lore-type: event
    unlock-behavior: on-analysis
-->
### The Treaty of Dust

**Lategame**: Recovered from an etched metal plate buried beneath the remains of the Bogotá embassy vault.

> "Signed by ash and sealed in absence, this was the final pact before the Fall. It wasn't between nations. It was between those who *knew*, and those who refused."

*If analyzed, the Living Chronicle marks a shift toward pre-collapse conspiracy theories, influencing faction-neutral survivor groups and vault-coded documents.*
<!-- /fragment-meta -->

<!-- fragment-meta
    affects-living-chronicles: true
    converts-myth-to-confirmed: true
    fragment-id: fragment-tower-that-didnt-fall
    fragment-timing: midgame
    fragment-lore-type: structure
    unlock-behavior: on-analysis
-->
### The Tower That Didn't Fall

**Midgame**: Coordinates engraved in the casing of a failed drone near the Gulf Scar.

> "One tower still blinks blue. Not red. Not dead. It records. It remembers. It refuses to admit the world ended."

*If analyzed, unlocks passive Living Chronicle note tracking "persistent structures" still active post-Fall and increases the chance of encountering stable zones.*
<!-- /fragment-meta -->
